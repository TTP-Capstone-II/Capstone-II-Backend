...
router.post("/auth0-login", async (req, res) => {
  try {
    ...
    if (!user && email) {
      // Try to find user by email if auth0Id not found
      user = await User.findOne({ where: { email } });

      if (user) {
        user.auth0Id = auth0Id;  // link auth0Id to existing user
        await user.save();
      }
    }
    ...
    if (!user) {
      ...
      console.log("Creating user with:", { auth0Id, email, finalUsername });
      console.log("passwordHash allowNull:", User.rawAttributes.passwordHash.allowNull);
